...
class SiteTving(SiteBase):
    ...
    def do_driver_action(cls, ins):
        WebDriverWait(ins.dm.driver, 5).until(lambda driver: driver.find_element_by_class_name('cjp-quality')).find_element_by_tag_name('button').click()
        WebDriverWait(ins.dm.driver, 5).until(
