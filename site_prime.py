...
class EntityPrime(EntityBase):
    ...
    def download_subtitle(self):
        ...
        for tmp1 in ['subtitleUrls']:
            for item in self.meta[tmp1]:
                ...
                #logger.debug(item)
                filepath = os.path.join(temp_dir, '{}.{}.{}.ttml'.format(self.code, item['type'], item['languageCode']))
    ...
    def download_mp4(self):
        for item in ['video', 'audio']:
            self.mp4_info[item]['original_filepath'] = os.path.join(temp_dir, '{}.{}.original.mp4'.format(self.code, item))
            self.mp4_info[item]['decryped_filepath'] = os.path.join(temp_dir, '{}.{}.decrypted.mp4'.format(self.code, item))
            self.mp4_info[item]['dump_filepath'] = os.path.join(temp_dir, '{}.{}.dump'.format(self.code, item))
            self.mp4_info[item]['info_filepath'] = os.path.join(temp_dir, '{}.{}.json'.format(self.code, item))
    ...
    def merge(self):
        ...
        self.mkv_filepath = os.path.join(temp_dir, '%s.mkv' % self.code)
            ...
            for filename in os.listdir(temp_dir):
                if filename.startswith(self.code) and filename.endswith('.srt'):
                    ...
                    if nation == 'ko':
                        ...
                        option += ['"%s"' % os.path.join(temp_dir, filename).replace(path_app_root, '.')]
                    else:
                        etc_option += ['--language', '"0:%s"' % nation, '"%s"' % os.path.join(temp_dir, filename).replace(path_app_root, '.')]
            option += etc_option
            logger.debug(option)
